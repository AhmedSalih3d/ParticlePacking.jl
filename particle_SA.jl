...
function read2DParticleGroup(filename::AbstractString)
    ...
    pg = [SVector{3}(pg[i][1],pg[i][2],pg[i][3]) for i in 1:(nCoords+ns1)]
    return pg,nCoords
end
#---------------------------- Some Constants ------------------------------------#
# Set some constants
...
function PackStep(pg,pg_tmp,u,u_tmp,nCoords,nTot,idxs)
    ...
    @fastmath @inbounds @threads for i = 1:nCoords
        ...
        dux = -BV * Wgx - DTZ * u_i[1];
        duz = -BV * Wgz - DTZ * u_i[3];
...
function RunAlgorithm()
    ...
    u = map(x -> x.*SA[1.0 0.0 -1.0]'.*A0,pg);
    ...
    idxs = inrange(balltree, pg, R, true)
